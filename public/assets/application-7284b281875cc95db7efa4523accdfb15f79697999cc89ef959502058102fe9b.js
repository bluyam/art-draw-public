...
(function() {
  var addClick, clickColor, clickDrag, clickX, clickY, colors, context, curColor, curColorIndex, exceedsSmallSquareBounds, lastStrokeClickCount, mobile, paint, ready, redraw, removeLastStroke, safeSplice;
  ...
  ready = function() {
    var canvas, canvasDiv, canvasHeight, canvasWidth, fbButton, gameClickX, i, j, leftMargin, ref, swatches, toolbar, topMargin, xBig, yBig;
    ...
    canvas.addEventListener('scrollstart', (function(e) {
      e.preventDefault();
    }), false);
    ...
    if (toolbar) {
      ...
      for (i = j = 0, ref = colors.length - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
    ...
    if (gameClickX) {
      if (document.getElementById("game_clickX").value !== "") {
        clickX = document.getElementById("game_clickX").value.split(',').map(function(item) {
          return parseInt(item, 10);
        });
        clickY = document.getElementById("game_clickY").value.split(",").map(function(item) {
          return parseInt(item, 10);
        });
        ...
        if ((xBig || yBig) && $(window).width() < 768) {
          clickX = clickX.map(function(item) {
            return item * 275 / 500;
          });
          clickY = clickY.map(function(item) {
            return item * 275 / 500;
          });
        }
        return redraw();
  ...
  exceedsSmallSquareBounds = function(e) {
    return e > 275;
  };
